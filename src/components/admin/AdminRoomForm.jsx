...
import FormInput from "./FormInput.jsx"; // Убедитесь, что путь корректен
import { HotelService } from "../../api/hotelApi";
import { RoomService } from "../../api/roomApi";

export default function AdminRoomForm({ onRoomSaved }) { // Изменено название пропса
    const { id } = useParams(); // Получаем ID комнаты, если это режим редактирования
    ...
    useEffect(() => {
        if (id) {
            setLoadingRoom(true);
            // Предполагается, что RoomService будет иметь метод getRoomById(id)
            roomService.getRoomById(id) // Этот метод нужно будет добавить
                .then(data => {
                    setTitle(data.title);
                    setPrice(data.price);
                    ...
                    setPrivateBathroom(data.privateBathroom);
                    setHasWifi(data.hasWifi);
                    setHasPrivatePool(data.hasPrivatePool);
                    setBeds(JSON.parse(data.beds)); // Предполагается, что beds хранится как JSON строка
                    setPreviewImageUrl(data.imageUrl); // Если есть URL существующего изображения
                })
                .catch(err => {
                    console.error("Error fetching room data:", err);
                    setErrorHotels("Failed to load room data.");
                })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let resultRoom;
            if (id) {
                // Предполагается, что updateRoom принимает id и FormData
                resultRoom = await roomService.updateRoom(id, formData); // Этот метод нужно будет добавить
                alert("Комната успешно обновлена!");
            } else {
                resultRoom = await roomService.createRoom(formData);
                alert("Комната успешно добавлена!");
            }

            if (onRoomSaved) {
                onRoomSaved(resultRoom);
            }
            navigate('/admin/rooms'); // Перенаправляем на список комнат

            // Сброс формы только при создании новой комнаты
            if (!id) {
                setTitle("");
                setPrice("");
                setCurrencyCode("USD");
                ...
                // setSelectedHotelId(""); // Не сбрасываем, если хотим сохранить выбор
            }
        } catch (error) {
            console.error("Error saving room:", error);
            alert("Ошибка при сохранении комнаты: " + (error.response?.data?.message || error.message));
        }
    };

    if (loadingHotels || loadingRoom) return <div className="p-6">Loading...</div>;
    if (errorHotels) return <div className="p-6 text-red-500">{errorHotels}</div>;

    return (
        <div className="rounded-[24px] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
            <div className="mb-5">
                <h2 className="text-[24px] font-bold text-[#1A1A1A]">{id ? "Edit Room" : "Add Room"}</h2>
                <p className="text-[14px] text-[#8A8A8A]">
                    {id ? "Edit the room information." : "Fill in the room information and add it to an existing hotel."}
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2">
                    <button
                        ...
                        disabled={loadingHotels || fetchedHotels.length === 0 || loadingRoom}
                    >
